...
FOLDER_PATH = "./utils"  # Change to the path of your folder
NAMESPACE = "utils"  # Default namespace (can be overridden per service)
    ...
    for doc in docs:
        ...
        for port_entry in ports:
            services.append({
                ...
                "node_port": port_entry.get("nodePort")  # May be None
            })
...
def main():
    ...
    # Sort services by nodePort if it exists, else push to end
    all_services.sort(key=lambda s: s["node_port"] if s["node_port"] is not None else float('inf'))
    ...
    for svc in all_services:
        ...
        if svc["node_port"] is not None:
            external_url = f"http://{NODE_IP}:{svc['node_port']}"
            print(f"  External URL : {external_url}")
        else:
            print(f"  External URL : Not exposed")
        print("-" * 60)
