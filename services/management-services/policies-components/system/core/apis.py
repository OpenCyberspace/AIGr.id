...
from .executor_proxy import execute_graph
...
def create_executor():
    try:
        ...
        data['executor_host_uri'] = 'http://' + data['cluster_public_ip'] + ':30900' 
...
@app.route("/function/get_logs/<function_name>", methods=["GET"])
def get_function_logs_proxy(function_name):
    try:
        tail = request.args.get("tail", default=None, type=int)

        function = FunctionsDB().read(function_name)
        executor_host_uri = function.function_executor_uri
        if not executor_host_uri:
            return jsonify({"success": False, "message": "Executor not found"}), 404

        client = ExecutorProxyClient(base_url=executor_host_uri)
        logs = client.get_logs(function_name, tail=tail)
        return jsonify({"success": True, "logs": logs}), 200

    except Exception as e:
        logging.error(f"Error getting logs for function: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
