...
from .block_scheduling import run_for_each_cluster
...
class DryRunExecutor:
    ...
    def execute(self, parameters, block_data):
        try:
            ...
            print('filter_logic:', filter_logic)
            search_api = SearchServerAPI()
            filter_result = search_api.filter_search(filter_type, filter_logic)
...
class BlockCreator:
    ...
    def execute(self):
        if self.mode == "select_clusters":
            ...
            responses = run_for_each_cluster(clusters, push_payload)
