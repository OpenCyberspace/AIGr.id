...
def redis_source_validator(values):

    keys_required = [('host', str), ('port', int), ('db', int), ('password', str), ('queue_prefix', str), ('queue_names', list), ('includes_meta', bool), ('size_padding', int)]
...
class RedisSource(BaseSourceType):
    ...
    def __init__(self, env, settings = {}, writer = None):
        ...
        print('Queues assigned ', self.queues)
    ...
    def __generate_key(self, part_name, seq_number = None,source_id = None):

        part_key = "{}__red__{}__{}".format(
            self.env.key_prefix,
            self.source_id if not source_id else source_id,
            part_name
        )

        return "{}__{}".format(part_key, seq_number)
    ...
    def runner(self):
        ...
        #create a redis connection:
                ...
                logging.info("Started redis task")
                ...
                while True :
                    ...
                    try:
                        self.__pause_lock()
                        key, data = connection.brpop(self.queues)
                        ...
                        data, metadata = self.parse_data(data)

                        if 'is_last_frame' in metadata and metadata['is_last_frame']:
                            return True, self.status
                        
                        source_id = self.source_id if 'source_id' not in metadata else metadata['source_id']
                        ...
                        should_validate = self.env.enable_validation and self.settings['includes_meta']

                        if should_validate and (not self.is_valid_frame(metadata)):
                            if self.env.enable_corrupt_persistence:
                                frame_key = self.__generate_key(key, seq_number = seq_number, source_id = source_id),
